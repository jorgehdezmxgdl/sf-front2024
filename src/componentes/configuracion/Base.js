...
export default function Base(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (option === 1) {
      ...
      await axios
        .post("http://127.0.0.1:5784/departamentos", data)
        .then((response) => {
      ...
      console.log(data);
      await axios
        .put("http://127.0.0.1:5784/departamentos/",data)
        .then((response) => {
          if (response.status === 200) {
            handleClickShopenClose();
  ...
  const columns = [
    ...
    {
      field: "activo",
      headerName: "Estatus del departamento",
      width: 250,
 ...
 const fetchData = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:5784/departamentos");
      if (response.status === 200) {
        if (response.data) {
